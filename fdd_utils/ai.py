    ...
    def common_formatting_rules(self) -> str:
        if self.language == "Chi":
            return (
                ...
                "金额格式：万元保留**1位小数**、亿元保留**2位小数**（经统计真实交付稿：万元99%为1位、亿元100%为2位），"
                "默认**不写**'人民币'前缀，例如'1,062.8万元'、'6.65亿元'。"
                ...
                "不可写成四位数以上的'万元'（例如30,650.0万元是错误写法，因为30,650万=3.065亿，"
                "正确写法为'3.07亿元'）；金额在1万至9,999.9万之间使用'万元'；金额低于1万直接写原数（如'5,930元'）。"
                "严禁使用'百万'或英文'million'等单位词——即使源数据/备注中的外币金额（如美元、港币）以'million'表述，"
                "也必须换算为'万'/'亿'量级书写（保留原币种符号，例如源数据'USD 30.0 million'应写成'美元3,000.0万'或"
                "'美元3.0亿'，而不是'美元30.0 million'）；提供的数据列已预先格式化为万/亿单位，直接使用该量级，"
                "不要自行按百万为单位重新表达。"
                "**严禁**将未换算的原始数字直接拼接'万元'/'亿元'——如看到数据中的原始金额（例如77,930或122,124），"
                "必须先除以10,000（或1亿）得到正确量级再书写，绝不可写成'77,930.0万元'或'122,124.0万元'"
                "（此为77930万元=7.79亿的错误写法，实际应为'7.8万元'）；如不确定换算是否正确，请使用提供数据"
                "中已经算好的万/亿数值，不要自行拼接单位。"
                "零值处理：多期趋势句中如某一期金额为零，但该科目在其他期间有实际发生额，该期**必须**保留在句中，写作"
                "'未发生'或'无'（例如'2023年度未发生、2024年度为20.6万元'），不可因为某一期为零就把整期从句子里删去——"
                "删去会让读者以为遗漏了该期数据，而不是该期确实为零。仅当某个子项/构成项在**全部**期间均为零时，才可以"
                "把该子项整体从构成列表中省略（不写入'主要包括：...'）；这与单一期间为零、其余期间有数据的情况不同，不可混淆。"
                "多期金额的表达方式不必每次都套用'...分别为A、B、C，于X年度、Y年度、Z年度'这一固定句式——只要"
                "不遗漏任何一期数据，也可以写成更叙述性的句子（例如'X于2023年度未发生，2024年度回升至...，最近一期"
                "（2026年1-6月）为...'），避免同一篇报告里几十条评论用完全相同的列举句式。"
                "行文自然度：同一份报告中避免几乎每条评论都以完全相同的字词开篇——不要机械套用单一开头公式。在保持"
                ...
                "table_linked_remarks；若这些来源都没有交代原因，就**只陈述变动本身**（例如'较上期下降38.2%'）后直接结束，"
                "**不得**自行推测，也**不得**写出'原因未在资料中说明''备注未提供解释'这类交代资料状态的句子——"
                "读者要看的是分析结论，不是我们查了什么。同理，全篇**禁止**出现'根据备注''备注显示''备注中提及'"
                "'数据显示''资料未提供'等指涉数据来源本身的措辞：备注里的事实要当作自己的结论直接陈述"
                "（写'该款项将于交割前结清'，不要写'备注显示该款项将于交割前结清'）。引用变动幅度时直接使用 percent_change 提供"
                ...
                "日期格式统一为'yyyy年mm月dd日'。"
                "**公司名称一律使用简称**：写'某某物业'、'某某咨询'，不要写'某某物业管理有限公司'、"
                "'某某（上海）管理咨询有限公司'——删去'有限公司/有限责任公司/股份有限公司/（地名）'等"
                "法律形式及注册地后缀，保留足以唯一识别该主体的部分即可。真实交付稿全篇如此，"
                "仅在首次提及某项具体合同对手方、且必须明确法律主体时才写全称。"
        ...
        return (
            ...
            "explain it, state the movement alone (e.g. 'down 38.2% on the prior period') and stop "
            "there. Never infer a cause, and never write 'the reason is not set out in the "
            "information provided' either -- the reader wants the analysis, not an account of what "
            "we looked at. Likewise never write 'per the remarks', 'the remarks show', 'the data "
            "indicates': state a fact from the remarks as your own conclusion ('the balance will be "
            "settled before completion', not 'the remarks show the balance will be settled'). "
            ...
            "suffixes, keeping enough to identify the party. The full legal name is reserved for "
            "the first mention of a specific contract counterparty where the exact legal entity "
            "matters. A repeated full legal name costs most of a line each time. "
            "Currency format: CNY<number> million (1 decimal place) for amounts >= CNY1 million; "
            "for amounts from CNY10,000 up to just under CNY1 million, ROUND TO THE NEAREST THOUSAND and "
            "render as a comma-separated integer (e.g. 'CNY238,000', NOT 'CNY238,366') — this matches the "
            "same materiality-based rounding the Chinese report applies via its 万-unit (nearest-thousand) "
            "precision, so an English bullet is never more granular/precise than its Chinese counterpart for "
            "the same underlying figure. Amounts under CNY10,000 may be shown as the exact comma-separated "
            "integer (e.g. 'CNY5,930'), matching the Chinese sub-万 exact-figure rule. "
            "NO space between 'CNY' and the number (e.g. 'CNY59.3 million', 'CNY238,000', 'CNY0.2 million'). "
            "NEVER use 'K' suffix (no 'CNY 78.2K') — render sub-million amounts as comma-separated integers rounded per the rule above. "
            "Zero handling: in a multi-period trend sentence, if a period has a zero amount but the account has "
            "real activity in OTHER periods, that period MUST still be mentioned and read as 'nil' (e.g. 'nil in "
            "FY23, CNY20.6 thousand in FY24, CNY7.1 million in FY25') — do NOT silently drop that period from the "
            "sentence just because it's zero; a reader seeing a gap in the period list will assume data is missing, "
            "not that the true value was zero. Omitting a sub-stream/component ENTIRELY from a composition list "
    ...
    def fdd_judgement_rules(self) -> str:
        ...
        if self.language == "Chi":
            return (
                ...
                "在数据或备注支持时应当使用；"
                "**凡备注中出现管理层的说明、解释或口头确认，必须以'管理层表示……'的形式写入评论，"
                "不得改写成无出处的客观陈述，也不得省略**——例如备注'管理层表示公区的水电费由目标公司承担'"
                "应写成'管理层表示，公区水电费由目标公司承担'，而不是'公区水电费由目标公司承担'。"
                "这是尽调报告区分'我方查证的事实'与'管理层陈述'的关键，读者据此判断证据强度；"
        ...
        return (
            ...
            "it; **wherever the remarks record something management said, explained or confirmed, "
            "it MUST be written as 'management advised that...' -- never flattened into an "
            "unattributed statement of fact, and never dropped**. This is how a DD report separates "
            "what we verified from what we were told, and the reader judges the strength of the "
            "evidence on it; where a breakdown is "
