    ...
    def _detail_table_guidance(df: Optional[pd.DataFrame], language: str) -> str:
        ...
        if language == "Chi":
            return (
                ...
                "然后以'明细如下：'收尾交由表格列示。"
                "**篇幅硬性要求：本科目整段不超过约120字。**真实交付稿这类段落为40-120字，"
                "实测本工具此前写到250-370字——原因是先把每期金额逐一列完、再补一句'明细如下'，"
                "等于表格与正文各写一遍。"
                "**明确禁止**：不得罗列各期总额（如'2023年度、2024年度、2025年度分别为A、B、C'）；"
                "不得罗列各构成项的每期金额；不得写年化换算数。这些全部由表格承担。"
                "正文只负责回答'这个科目由什么构成、由谁提供、依什么计费'，"
                "以及若有重大变动则说明原因——数字留给表格。"
            )
        return (
            ...
            "relevant, and then handing off with 'the breakdown is set out below'. "
            "**Hard limit: keep this whole paragraph under about 120 words.** The real "
            "deliverable runs 40-120 for these; this tool was measured at 250-370 because it "
            "listed every period's figure first and only then added the handoff -- writing the "
            "table twice. **Explicitly forbidden**: listing period totals ('FY23, FY24 and FY25 "
            "were A, B and C'), listing each component's per-period figures, and annualised "
            "restatements. The table carries all of those. The prose answers only what the "
            "account consists of, who provides it and on what basis, plus the reason for any "
            "material movement -- the figures belong to the table."
        )
