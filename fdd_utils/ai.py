    ...
    def common_formatting_rules(self) -> str:
        if self.language == "Chi":
            return (
                ...
                "必须先除以10,000（或1亿）得到正确量级再书写，绝不可写成'人民币77,930.0万元'或'人民币122,124.0万元'"
                "（此为77930万元=7.79亿的错误写法，实际应为'人民币7.8万元'）；如不确定换算是否正确，请使用提供数据"
                "中已经算好的万/亿数值，不要自行拼接单位。"
                "零值处理：多期趋势句中如某一期金额为零，但该科目在其他期间有实际发生额，该期**必须**保留在句中，写作"
                "'未发生'或'无'（例如'2023年度未发生、2024年度为人民币20.6万元'），不可因为某一期为零就把整期从句子里删去——"
                "删去会让读者以为遗漏了该期数据，而不是该期确实为零。仅当某个子项/构成项在**全部**期间均为零时，才可以"
                "把该子项整体从构成列表中省略（不写入'主要包括：...'）；这与单一期间为零、其余期间有数据的情况不同，不可混淆。"
                "多期金额的表达方式不必每次都套用'...分别为人民币A、B、C，于X年度、Y年度、Z年度'这一固定句式——只要"
