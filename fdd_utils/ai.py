    ...
    def _detail_table_guidance(df: Optional[pd.DataFrame], language: str) -> str:
        ...
        if language == "Chi":
            return (
                ...
                "禁止编造）**：紧接在'明细如下：'之后另起一行，针对1-3个真正有实质说明可写的构成项"
                "（服务提供方是谁、依什么基准计费、合同条款为何——不是每个构成项都需要，通常只有"
                "少数几个有这类背景信息），各写一条以'➢ [构成项名称]：'开头的说明。"
                "每条不超过约150字，第二段全部加起来不超过约400字。\n"
        ...
        return (
            ...
            "a new line right after the part-1 handoff, one bullet per component (up to 3, not "
            "every component needs one -- usually only a few have this kind of background) that "
            "genuinely has supporting detail, each starting with '- [component name]: ', "
            "covering who provides it, the charging basis, or contract terms. Keep each under "
            "about 150 words, and the whole of part 2 under about 400 words.\n"
