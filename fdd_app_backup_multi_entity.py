...
def main():
    ...
    if uploaded_file is not None:
        ...
        # Process all three entities
        config, mapping, pattern, prompts = load_config_files()
        
        # Define all entities to process
        all_entities = [
            {'name': 'Haining', 'suffixes': ['Wanpu', 'Limited']},
            {'name': 'Nanjing', 'suffixes': ['Wanchen', 'Limited', 'Logistics', 'Development', 'Supply', 'Chain']},
            {'name': 'Ningbo', 'suffixes': ['Wanchen', 'Limited', 'Logistics', 'Development', 'Supply', 'Chain']}
        ]
        ...
        for entity_info in all_entities:
            ...
            entity_suffixes = entity_info['suffixes']
            
            # Handle different statement types with session state caching
            if statement_type == "BS":
                # Create cache key for this entity
                cache_key = f"sections_by_key_{uploaded_file.name if hasattr(uploaded_file, 'name') else 'default'}_{entity_name}"
                
                if cache_key not in st.session_state:
                    # Process this entity
                    with st.spinner(f"🔄 Processing {entity_name}..."):
        ...
        if entity_names:
            ...
            for i, (entity_name, entity_data) in enumerate(all_entities_data.items()):
                with tabs[i]:
                    ...
                    if statement_type == "BS":
                        ...
                        st.markdown(f"### Income Statement - {entity_name}")
                        st.info("📊 Income Statement processing will be implemented here.")
                    elif statement_type == "ALL":
                        st.markdown(f"### Combined Financial Statements - {entity_name}")
        ...
        if uploaded_file is not None and st.session_state.get('all_entities_data'):
            try:
                ...
                for entity_name, entity_data in all_entities_data.items():
                    ...
                    if not filtered_keys_for_ai:
                        st.warning(f"No data found for AI processing with the selected statement type for {entity_name}.")
                        continue
                    ...
                    all_entities_ai_data[entity_name] = {
                ...
                if run_ai_clicked:
                    ...
                    try:
                        ...
                        for i, (entity_name, entity_ai_data) in enumerate(all_entities_ai_data.items()):
                            status_text.text(f"🤖 Processing {entity_name} ({i+1}/{total_entities})...")
                            ...
                            all_agent1_results[entity_name] = agent1_results
                ...
                if run_proof_clicked:
                    ...
                    try:
                        ...
                        if not agent1_results_all:
                            ...
                            total_entities = len(all_entities_ai_data)
                            for i, (entity_name, entity_ai_data) in enumerate(all_entities_ai_data.items()):
                                status_text.text(f"🧐 Proofreading {entity_name} ({i+1}/{total_entities})...")
                                progress_bar.progress(10 + (i * 80 // total_entities))
                                
                                entity_agent1_results = agent1_results_all.get(entity_name, {}) or {}
                                if entity_agent1_results:
                                    ...
                                    all_proof_results[entity_name] = proof_results
                ...
                if run_both_clicked:
                    ...
                    try:
                        ...
                        total_entities = len(all_entities_ai_data)
                        for i, (entity_name, entity_ai_data) in enumerate(all_entities_ai_data.items()):
                            status_text.text(f"🤖 Generating content for {entity_name} ({i+1}/{total_entities})...")
                        ...
                        for i, (entity_name, entity_ai_data) in enumerate(all_entities_ai_data.items()):
                            ...
                            entity_agent1_results = all_agent1_results.get(entity_name, {}) or {}
                            if entity_agent1_results:
        ...
        if any_agent_completed and st.session_state.get('all_entities_ai_data'):
            ...
            for i, (entity_name, entity_ai_data) in enumerate(all_entities_ai_data.items()):
                with ai_tabs[i]:
                    ...
                    entity_agent1_results = agent1_results_all.get(entity_name, {}) or {}
                    entity_agent3_results = agent3_results_all.get(entity_name, {}) or {}
            ...
            try:
                ...
                if not template_path:
                    ...
                    for entity_name, entity_data in all_entities_data.items():
                        ...
                        all_entity_content[entity_name] = {
                            'keys': filtered_keys,
                            'sections_by_key': sections_by_key,
                            'base': entity_base
                        }
                    ...
                    for entity_name, entity_content in all_entity_content.items():
