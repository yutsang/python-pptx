...
def run_simple_proofreader(english_results, ai_data, progress_callback=None, language='english'):
    ...
    try:
        ...
        # Get detected language from session state or use parameter
        detected_language = st.session_state.get('detected_language', language)
        
        print(f"\n🔍 PROOFREADER SETUP: Language={detected_language}")
        
        # Create proofreader agent with correct language
        proofreader = ProofreadingAgent(
            use_local_ai=st.session_state.get('use_local_ai', False),
            use_openai=st.session_state.get('use_openai', False),
            language=detected_language  # Use detected language from session or parameter
        )
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if generate_report_clicked:
            ...
            try:
                # ========================================
                # UNIFIED WORKFLOW (Handles both Chinese and English)
                # ========================================
                is_chinese = detected_language == 'chinese'
                ...
                if not english_results:
                    ...
                    st.error("❌ 中文内容生成失败" if is_chinese else "❌ Content generation failed")
                    return
                ...
                def progress_callback_proof(p, msg):
                    ...
                    if msg and isinstance(msg, str):
                        if 'Processing' in msg and '•' in msg:
                            ...
                            current_key = msg.replace('🔄 Processing', '').strip()
                        elif len(msg.strip()) < 50:
                            current_key = msg.strip()

                    key_index = int(p * total_keys) if p > 0 else 0
                    status_display = f"🧐 校对内容... ({key_index}/{total_keys}) - {current_key}"
                    status_text.text(status_display)
                    progress_bar.progress(0.3 + p * 0.2)
                ...
                if detected_language == 'chinese':
                    ...
                    proofread_results = run_simple_proofreader(english_results, temp_ai_data, progress_callback=progress_callback_proof_eng, language=detected_language)
