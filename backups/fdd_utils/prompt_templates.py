...
1. 翻译成简体中文
2. 保留所有数字、百分比和货币符号
3. 将英文数字单位转换为中文单位，并自动调整数量级：
   - 小金额：k (thousand) → 万 (wan)
   - 中等金额：m/M (million) → 百万 (bai wan) 或 佰万
   - 大金额：自动转换为亿 (yi) 单位
   - 例如：100k → 100万, 50m → 5000万, 158m → 1.58亿, 2b → 20亿
   - 特别注意：超过1亿的百万单位应转换为亿单位（如：158百万 → 1.58亿）
4. 保持专业语气
5. 只返回翻译结果，不要添加解释""",
